...
void timer0SetPrescaler(u08 prescale)
{
	...
	outp( (inb(TCCR0) & ~TIMER_PRESCALE_MASK) | prescale, TCCR0);
}

void timer1SetPrescaler(u08 prescale)
{
	// set prescaler on timer 1
	outp( (inb(TCCR1B) & ~TIMER_PRESCALE_MASK) | prescale, TCCR1B);
}
...
void timer2SetPrescaler(u08 prescale)
{
	...
	outp( (inb(TCCR2) & ~TIMER_PRESCALE_MASK) | prescale, TCCR2);
}
#endif
...
// Starts the 16-bit timer, use for timerGetMS
void timerStart(void)
{
	outb(TCNT1H, 0x00); //need to write to high byte first
	outb(TCNT1L, 0x00);
}

// will sort of work, assumes a certain prescaling factor
u16 timerGetMS(void)
{
	u08 temp;
	u16 timerVal;
						 		//need to get low byte first
	timerVal = TCNT1L; 		//read low byte directly
	temp = TCNT1H;			//read high byte into temp variable
	timerVal += (temp << 8);	//stick high byte into high byte of result
	return (u16)(((u32)timerVal*1000UL)/7812); 		//7812.5 tick per sec
	//Note that this takes more than a timer tick by itself, possibly 2
}
