...
/*******************************************************
* uartGetBuffer()
* Reads data from the serial port until it encounters a
* newline or up to maxbytes. Stores it in buf.
*
* Returns: Number of bytes read
*******************************************************/
u16 uartGetBuffer(char *buf, u16 maxlength)
{
	char byte; // temporary storage for byte
	
	u16 i = 0;
	
	while( (byte = (char)uartGetByte()) != -1)
	{
		buf[i] = byte;		
		i++;
